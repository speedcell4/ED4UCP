...
from experiments.constants import MBR_PATH
...
from experiments.ensemble_guide import Bests
from experiments.ensemble_guide import Worsts
...
def main():
    ...
    if config.Run is not None:
        for run in config.Run:
            ensembles = ensemble_by_model_run(dict(guide.loc[int(run)]), file_name=config.file_name,
                                              MBR_mode=config.MBR_mode)
