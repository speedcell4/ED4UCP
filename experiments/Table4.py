...
from experiments.constants import ENTROPIES
...
def table_4():
    ...
    for distillation_method in distillation_methods:
        ...
        entropies = [np.mean(list(map(float, open(
            os.path.join(MOTHER_PATH, ENTROPIES, distillation_method, run)).read().strip().split('\n')))) for run in
                     runs]
