...
from experiments.figure1_guide import BATCHES
from experiments.figure1_guide import MODEL_RENAMING
...
def figure_1():
    ...
    def plot_one_group(ax, start, models, overal, color, line, hatch, label, text_only_on=None, annotate=None,
                       denoising_overal=None):
        ...
        if annotate == 'denoising':
            ax.annotate("", xy=(start + (len(models) - 1) * bar_width, max([m[1] for m in models]) - .5),
                        xytext=(start + (len(models) - 1) * bar_width, overal + .3),
            ...
            ax.annotate(
                ...
                fontsize=10, color=annotations_color, fontstyle='italic')
        if annotate == 'util':
            ax.annotate("", xy=(start - .5 * bar_width, denoising_overal - 0.5),
                        xytext=(start - .5 * bar_width, overal + .5),
                        arrowprops=dict(arrowstyle="<->", lw=1, color=annotations_color))
            ax.annotate(
                'Additional\nboost\ngained by\nutilizing\nexpertise',
    ...
    for i, ((denoising_models, denoising_overal), (utilizing_models, utilizing_overal)) in enumerate(data):
        ...
        start, denoising_bar = plot_one_group(ax, start, denoising_models, denoising_overal, '#daedfe', 'dotted', '/',
                                              'Ensemble of different runs', annotate='denoising' if is_last else None)
        start, utilizing_bar = plot_one_group(ax, start, utilizing_models, utilizing_overal, '#dafcd9', 'dashdot',
                                              '....', 'Ensemble of different models',
                                              annotate='util' if is_last else None, denoising_overal=denoising_overal)
        start += bar_width / 2
    plt.xticks([2.5 * bar_width + 6.5 * i * bar_width for i in range(7)], [f'Group {i + 1}' for i in range(7)],
               fontsize=13)
