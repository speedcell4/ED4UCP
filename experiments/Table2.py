...
from experiments.constants import COMB_BEST_NAME
from experiments.constants import COMB_WORST_NAME
...
from experiments.constants import RNNG_TEST_FILE_NAME
...
from experiments.constants import UNION_PATH
from experiments.constants import URNNG_TEST_FILE_NAME
...
def table_2():
    ...
    for i, model in enumerate(guide.columns):
        ...
        rnng_f1 = sentence_level_f1(golds, open(
            os.path.join(MOTHER_PATH, model, guide.loc[run1, model], RNNG_TEST_FILE_NAME)).readlines())
        urnng_f1 = sentence_level_f1(golds, open(
            os.path.join(MOTHER_PATH, model, guide.loc[run1, model], URNNG_TEST_FILE_NAME)).readlines())
        print(f' {i + 3}  {model}\t\t' + (
            "\t" if len(model) < 12 else "") + f'| {mean}±{std}\t| {run1_f1}\t {rnng_f1}\t {urnng_f1}')
    print('-' * 72)

    union_rnng_f1 = sentence_level_f1(golds,
                                      open(os.path.join(MOTHER_PATH, UNION_PATH, RNNG_TEST_FILE_NAME)).readlines())
    union_urnng_f1 = sentence_level_f1(golds,
                                       open(os.path.join(MOTHER_PATH, UNION_PATH, URNNG_TEST_FILE_NAME)).readlines())
    ...
    rnng_f1 = sentence_level_f1(golds, open(
        os.path.join(MOTHER_PATH, MBR_SEL_PATH, RUN + str(run1), RNNG_TEST_FILE_NAME)).readlines())
    urnng_f1 = sentence_level_f1(golds, open(
        os.path.join(MOTHER_PATH, MBR_SEL_PATH, RUN + str(run1), URNNG_TEST_FILE_NAME)).readlines())
    print(f' {len(guide.columns) + 4} {MBR_SEL_PATH}\t\t' + (
        "\t" if len(MBR_SEL_PATH) < 12 else "") + f'| {mean}±{std}\t| {run1_f1}\t {rnng_f1}\t {urnng_f1}')

    mean, std, run1_f1 = mean_std_run1(golds, MBR_GEN_PATH, [RUN + str(i) for i in guide.index])
    rnng_f1 = sentence_level_f1(golds, open(
        os.path.join(MOTHER_PATH, MBR_GEN_PATH, RUN + str(run1), RNNG_TEST_FILE_NAME)).readlines())
    urnng_f1 = sentence_level_f1(golds, open(
        os.path.join(MOTHER_PATH, MBR_GEN_PATH, RUN + str(run1), URNNG_TEST_FILE_NAME)).readlines())
    print(f' {len(guide.columns) + 5} {MBR_GEN_PATH}\t\t' + (
        "\t" if len(MBR_GEN_PATH) < 12 else "") + f'| {mean}±{std}\t| {run1_f1}\t {rnng_f1}\t {urnng_f1}')
    ...
    for i, comb in enumerate([COMB_WORST_NAME, COMB_BEST_NAME]):
        ...
        rnng_f1 = sentence_level_f1(golds, open(
            os.path.join(MOTHER_PATH, MBR_GEN_PATH, comb, RNNG_TEST_FILE_NAME)).readlines())
        urnng_f1 = sentence_level_f1(golds, open(
            os.path.join(MOTHER_PATH, MBR_GEN_PATH, comb, URNNG_TEST_FILE_NAME)).readlines())
