...
from experiments.table1_guide import CORR_DIORA
from experiments.table1_guide import CORR_HETEROGENEOUS
...
def triangle(models):
    for i in range(len(models)):
        ...
        for j in range(i + 1):
            ...
            line.append(str(f1))
        print(f'{models[i][0]} ({models[i][1]})\t' + (
            "\t" if len(models[i][0] + models[i][1]) < 12 else "") + '| ' + '\t\t '.join(line))
    print('\t\t\t', '-' * 47)
